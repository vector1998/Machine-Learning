...
#para reducir el sobre ajuste reduciremos el valor de las tetas o pesos
#modificación:
#l=regularizacion("si es demasiado grande, causará un underfitting")
#aplha=tarifa de aprendizaje
#la regularización se aplica para todas las tetas, excepto para teta 0
#0j:=0j(1-alpha*(l/m))-alpha*1/m(sumatoria[i,m]{(h(x^[i])-y^[i])*x^[i]}) donde j=1,2,3,4,..n
    

      def __init__(self,tetas,ejemplos,salidas,tarifaAprendizaje,regularizacion):
          self.tetas=tetas
          self.ejemplos=ejemplos
          self.y=salidas
          self.tarifaAprendizaje=tarifaAprendizaje
          self.cantidad_atributos=len(tetas)
          self.cantidadEjemplos=len(ejemplos)
          self.regularizacion=(1-(self.tarifaAprendizaje*(regularizacion/self.cantidadEjemplos)))
...
#función que nos permite actualizar cada valor de teta por separado
      def __actualizar_teta(self,functionHipotesis,teta_actualizar):
           ...
           pendiente=round(((self.tarifaAprendizaje*(1/self.cantidadEjemplos))
                          *self.__sumatoria(functionHipotesis,self.tetas,teta_actualizar)),10)
           print(pendiente)
           if teta_actualizar!=0:
                return round((self.tetas[teta_actualizar]*self.regularizacion)-pendiente,10)
           else:
...
DG=DescensoGradiente(tetas,ejemplos,y,0.003,0)
...
test=np.array([1,10,8])
print(f"la predicción es:{DG.predecir(DG.hipotesisLogisticR,test)}")
